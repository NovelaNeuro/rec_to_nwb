...
class TestFlElectrodeGroupManager(TestCase):
    ...
    def test_manager_builds_FlNwbElectrodeGroups_successfully(self):
        ...
        fl_nwb_electrode_groups = fl_electrode_group_manager.get_fl_nwb_electrode_groups(
            probes=probes
        )
    ...
    def test_manager_failed_builds_FlNwbElectrodeGroups_due_to_None_metadata(self):
        mock_probe_1 = Mock(spec=Probe)
        mock_probe_2 = Mock(spec=Probe)
        probes = [mock_probe_1, mock_probe_2]
        ...
        fl_electrode_group_manager.get_fl_nwb_electrode_groups(
            probes=probes
        )

    @should_raise(NoneParamException)
    def test_manager_failed_builds_FlNwbElectrodeGroups_due_to_None_probes(self):
        electrode_groups_metadata_1 = {'id': 0, 'location': 'mPFC', 'device_type': 'tetrode_12.5',
                                       'description': 'Probe 1'}
        electrode_groups_metadata_2 = {'id': 1, 'location': 'mPFC', 'device_type': '128c-4s8mm6cm-20um-40um-sl',
                                       'description': 'Probe 2'}
