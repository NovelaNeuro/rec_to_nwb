...
conda-build . -c acwikla-novela -c novelakrk -c conda-forge --no-include-recipe --python 3.7 || exit 1
...
anaconda upload $CONDA_BUILD_PATH/**/${PKG_NAME}-${VERSION}-py36_0.tar.bz2 --force || exit 1
anaconda upload $CONDA_BUILD_PATH/**/${PKG_NAME}-${VERSION}-py37_0.tar.bz2 --force || exit 1
