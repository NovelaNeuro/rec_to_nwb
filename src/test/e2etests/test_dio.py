...
from src.datamigration.nwb_builder.extractors.dio_manager import DioManager
...
class TestDio(unittest.TestCase):
    ...
    def test_dio_extractor(self):
        manager = DioManager([self.create_test_dataset()], self.metadata)
        timeseries = manager.get_dio()
        self.assertTrue(1)
